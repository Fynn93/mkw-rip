...
{
	FILE *header_nfo = fopen("/mkw-rip/header.bin", "wb");
	if (!header_nfo)
		return false;
	...
	FILE *main_dol = fopen("/mkw-rip/main.dol", "wb");
	if (!main_dol)
	{
		printf("Failed to open /mkw-rip/main.dol for write\n");
		free(dol);
		return false;
	}
...
int main(int argc, char **argv)
{
	...
	printf("\x1b[2;0H");
	printf("Mario Kart Wii Ripper v1.0\n\n");
...
reinsert_disc:
	...
	if (WDVD_LowReadDiskId() || memcmp((void*)0x80000000, "RMC", 3)) {
		printf("Sir, this is not Mario Kart Wii\n");
		goto reinsert_disc;
	}
	...
	printf("Press A to dump the Tracks or B to dump the entire disc.\n");
	...
	if (!DumpFolder(select ? "/" : "/Race/Course", select ? "/mkw-rip" : "/mkw-rip/Course")) {
